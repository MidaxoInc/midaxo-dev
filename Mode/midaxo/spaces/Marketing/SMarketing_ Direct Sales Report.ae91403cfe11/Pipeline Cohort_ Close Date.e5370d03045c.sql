with deal AS
  (SELECT distinct last_day(d.createdate,'month') AS createdate,
                   ...
                   count(d.deal_amount) over (partition BY last_day(d.createdate,'month'),last_day(d.closedate,'month')) AS dealcount,
...
SELECT d.ddate AS createdate,
       ...
       a.dealcount,
       ifnull(a.dealcount/a.cohort_dealcount,0) AS deals_closed
FROM datecohort d
left join deal a
