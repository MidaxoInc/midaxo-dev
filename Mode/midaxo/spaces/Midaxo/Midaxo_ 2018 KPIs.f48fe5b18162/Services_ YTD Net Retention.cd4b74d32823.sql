...
with churn AS
   ...
   WHERE a.value is not null
     AND a.metric in ('churn','upgrade','expansion')
   ORDER BY ddate ASC),
     monthly AS
...
SELECT distinct t.ddate,
                ...
                sum(m.arr) over (order BY t.ddate asc rows between unbounded preceding and current row) - sum(t.target) over (order BY t.ddate asc rows between unbounded preceding and current row) as gap
FROM target t
left join monthly m
  on t.ddate = m.ddate
